...
SET mapred.job.name=gwt_followtolike_event_3_{{ds}};

CREATE TABLE IF NOT EXISTS da.gwt_followtolike_event_3 (
os string,
group_id string, 
gender string,
group_name string,
like_click_source string,
pv bigint,
uv bigint
...
insert overwrite table da.gwt_followtolike_event_3 PARTITION(dt='{{ds}}')
...
(
SELECT  dt, actor_user_id, get_json_object(data_raw_data,'$.externalData.source') as like_click_source,
COUNT(1) as pv
FROM dwd.dwd_tantan_eventlog_reduced_i_d
WHERE  dt='{{ds}}' AND (name rlike 'like.click')
