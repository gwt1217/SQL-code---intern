...
SET mapred.job.name=gwt_followtolike_event_4_{{ds}};

CREATE TABLE IF NOT EXISTS da.gwt_followtolike_event_4 (
os string,
group_id string, 
gender string,
group_name string,
history_match_source string,
pv bigint,
uv bigint
...
insert overwrite table da.gwt_followtolike_event_4 PARTITION(dt='{{ds}}')
...
(
SELECT  dt, actor_user_id, get_json_object(data_raw_data,'$.externalData.source') as history_match_source,
COUNT(1) as pv
FROM dwd.dwd_tantan_eventlog_reduced_i_d
WHERE  dt='{{ds}}' AND (name rlike 'history.match.click')
