...
SET mapred.job.name=gwt_followtolike_event_1_{{ds}};

CREATE TABLE IF NOT EXISTS da.gwt_followtolike_event_1 (
os string,
group_id string, 
gender string,
group_name string,
view_product_type string,
pv bigint,
uv bigint
...
insert overwrite table da.gwt_followtolike_event_1 PARTITION(dt='{{ds}}')
select os, 
...
(
SELECT  dt, actor_user_id, get_json_object(data_raw_data,'$.externalData.productType') as view_product_type,
COUNT(1) as pv
FROM dwd.dwd_tantan_eventlog_reduced_i_d
WHERE dt='{{ds}}' AND (name rlike 'purchase_page.pageview'
