...
SET mapred.job.name=gwt_followtolike_event_7_{{ds}};

CREATE TABLE IF NOT EXISTS da.gwt_followtolike_event_7 (
os string,
group_id string, 
gender string,
group_name string,
...
insert overwrite table da.gwt_followtolike_event_7 PARTITION(dt='{{ds}}')
...
(
SELECT  dt, actor_user_id, get_json_object(data_raw_data,'$.externalData.matchSource') as match_source,
COUNT(1) as pv
FROM dwd.dwd_tantan_eventlog_reduced_i_d
WHERE  dt='{{ds}}' AND (name rlike 'user.moment.relationship')
GROUP BY dt, actor_user_id,get_json_object(data_raw_data,'$.externalData.matchSource')
)b
